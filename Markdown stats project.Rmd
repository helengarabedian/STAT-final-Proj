...
## Part D: Bootstrap and Histogram
```{r}
civilian <- astro1$`Space Flights`[astro1$Military_Status == "Civilian"]
military <- astro1$`Space Flights`[astro1$Military_Status == "Military"]

boot_diff <- replicate(10000, {
  sample_civ <- sample(civilian, replace = TRUE)
  sample_mil <- sample(military, replace = TRUE)
  mean(sample_mil) - mean(sample_civ)
})

quantile(boot_diff, c(0.025, 0.975))

hist(boot_diff,
     main = "Bootstrap Distribution: Difference in Mean Space Flights",
     xlab = "Military - Civilian",
     breaks = 30)
